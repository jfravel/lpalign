...
% Build from Documentation/ with:
% TEXINPUTS=..: pdflatex lpalign-Documentaion.tex
\usepackage{lpalign}
...
Also available is \verb|\subref| and \verb|\subeqref| which work like \verb|\ref| and \verb|\eqref| but only reference the secondary, sub-index of the tag. In most cases, ordinary \verb|\label| is enough. The \verb|\sublabel| macro remains available as a compatibility alias for \verb|\label|.
\begin{tcolorbox}
  \begin{verbatim}
...
or just \subeqref{sq:1} or even \eqref{sq:2}.
\end{tcolorbox}
\verb|\subref| and \verb|\subeqref| are intended to work with both \verb|namedsubeqs| and traditional \verb|subequations| environments when the equations are labeled with ordinary \verb|\label|.
...
\begin{itemize}
	...
	\item[$\boxtimes$] The \verb|\sublabel|, \verb|\subref| and \verb|\subeqref| macros previously depended on a custom labeling path and did not work reliably in traditional \textsf{subequations}. Fixed by storing sublabels automatically from ordinary \verb|\label| inside subequation environments; test branch after v1.2.
