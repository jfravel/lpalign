...
\AtBeginDocument{%
  \@ifpackageloaded{hyperref}{%
    \hypersetup{%
      pdftitle={The lpalign Package},
      pdfauthor={Jamie Fravel}%
    }%
  }{}%
}
\makeatother
...
Options are given as key-value pairs in the optional argument. The currently supported keys are \verb|style| for the second-index formatter (default \verb|\alph|) and \verb|punct| for the separator between the main tag and the sub-tag (default \verb|.|).
For backward compatibility, a single optional argument without an \verb|=| sign is still interpreted as the style argument.
    \begin{itemize}
    \item[M.] a name or abbreviation.
    \item[O.] optional key-value settings such as \verb|style=\roman| or \verb|punct=-|.
    \end{itemize}
...
\verb|punct| changes the separator between the main tag and the sub-tag.
\begin{tcolorbox}
  \begin{verbatim}
  \begin{namedsubeqs}{EQ}[style=\roman,punct=-]
  \end{verbatim}
\begin{center}$\downarrow$\end{center}
\VTrg
    \begin{namedsubeqs}{EQ}[style=\roman,punct=-]
...
Also available is \verb|\subref| and \verb|\subeqref| which work like \verb|\ref| and \verb|\eqref| but only reference the secondary, sub-index of the tag. Inside both \verb|namedsubeqs| and traditional \verb|subequations|, ordinary \verb|\label| is enough. The \verb|\sublabel| macro remains available.
\begin{tcolorbox}
  \begin{verbatim}
...
or just \subeqref{sq:1} or even \eqref{sq:2}.
\end{tcolorbox}
\verb|\subref| and \verb|\subeqref| work with ordinary \verb|\label| inside both \verb|namedsubeqs| and traditional \verb|subequations| environments.

\begin{tcolorbox}
  \begin{subequations}\label{test}
  \begin{align}
    A\mathbf{x} = \mathbf{b}   \label{test:1}\\
    C\mathbf{y} = \mathbf{d}   \label{test:2}
  \end{align}
  \end{subequations}
Testing subrefs with subequations: \verb|\ref{test}| \ref{test}, \verb|\ref{test:1}| \ref{test:1}, \verb|\subref{test:1}| \subref{test:1}, \verb|\subeqref{test:2}| \subeqref{test:2}
\end{tcolorbox}
...
\section{Version History}
	...
	\item[v1.2] Jun. 25, 2026. Bug fix. Removed unused package dependencies, fixed the internal \verb|\@currentlabel| handling in \textsf{namedsubeqs}, added a no-\textsf{hyperref} fallback for \verb|\subref| and \verb|\subeqref|, and cleaned up documentation inconsistencies.
	\end{itemize}
