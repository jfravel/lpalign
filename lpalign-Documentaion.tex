...
Currently supported optional keys include 
	...
	\item \verb|before| -- arbitrary code placed after the indent and before the title so that it stays flush with the title. The older \verb|preamble| spelling is still accepted as a compatibility alias.
	\item \verb|after| -- arbitrary code placed after the printed abbreviation.
	\item \verb|flush| -- centers or right flushes the paragraph title via \verb|center| or \verb|right| -- respectively.
	\item \verb|cref| -- passes a label type to \verb|\refstepcounter| as the reference type override for that paragraph if \textsf{cleveref} is loaded. If you use a custom category, define it with \verb|\crefname| as in the example below.
	\end{itemize}

\begin{tcolorbox}
  \begin{verbatim}
\namedpara{Indented Paragraph}[abv=indent, indent=6em]

\namedpara{Before Paragraph}
                    [abv=bef, before=$\sim\sim\sim$]

\namedpara{After Paragraph}
                   [abv=aft, after=$\leadsto$]

\namedpara{Rightflushed Paragraph}[abv=right, flush=right]

\namedpara{Cref Satisfiability}
                        [abv=SAT, cref=Problem]\label{sat}

If loaded, \textsf{cleveref} uses the \verb|cref| key to 
reference the paragraph \cref{sat}. This example assumes that the
\verb|Problem| category has been defined with \verb|\crefname|.
  \end{verbatim}
\begin{center}$\downarrow$\end{center}\Vskp
\namedpara{Indented Paragraph}[abv=indent, indent=6em] \\
\namedpara{Before Paragraph}[abv=bef, before=$\sim\sim\sim$] \\
\namedpara{After Paragraph}[abv=aft, after=$\leadsto$] \\
\namedpara{Rightflushed Paragraph}[abv=right, flush=right] \\
\namedpara{Cref Satisfiability}[abv=SAT, cref=Problem]\label{sat} \\
If loaded, \textsf{cleveref} uses the \verb|cref| key to reference the paragraph \cref{sat}
\end{tcolorbox}
...
Also available are \verb|\subref| and \verb|\subeqref|, which work like \verb|\ref| and \verb|\eqref| but reference only the secondary, sub-index part of a tag inside either \verb|namedsubeqs| or a traditional \verb|subequations| environment from the \textsf{amsmath} package.
\begin{tcolorbox}
  \begin{verbatim}
...
\section{Version History}
	...
	\item[v1.2] Jun. 25, 2026. Bug fix. Removed unused package dependencies, fixed the internal \verb|\@currentlabel| handling in \textsf{namedsubeqs}, added a no-\textsf{hyperref} fallback for \verb|\subref| and \verb|\subeqref|, and added \verb|\subref|/\verb|\subeqref| support for the traditional \textsf{subequations} environment from \textsf{amsmath}.
	\end{itemize}
