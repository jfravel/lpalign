...
Also available is \verb|\subref| and \verb|\subeqref| which work like \verb|\ref| and \verb|\eqref| but only reference the secondary, sub-index of the tag. To use them, label subequations with \verb|\sublabel|.
\begin{tcolorbox}
  \begin{verbatim}
...
or just \subeqref{sq:1} or even \eqref{sq:2}.
\end{tcolorbox}
\verb|\subref| and \verb|\subeqref| use the sub-index stored by \verb|\sublabel| inside \verb|namedsubeqs|.
...
\begin{lpalign}{Minimize}{f(\mathbf{x})    \sublabel{NLP.0}}
  g_i(\mathbf{x}) &\GE 0 &&\forall\ i\in I \sublabel{NLP.1} \\&&
  h_j(\mathbf{x}) &\EQ 0 &&\forall\ j\in J \sublabel{NLP.2} \\&&
...
\begin{itemize}
	...
	\item[$\boxdot$] The \verb|\subref| and \verb|\subeqref| macros currently rely on \verb|\sublabel| to store sub-index information explicitly. Automatic support for ordinary \verb|\label| in all subequation settings remains unresolved after v1.2 testing.
