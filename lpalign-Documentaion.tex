...
\namedpara{Cref Satisfiability}[abv=SAT]\label[problem]{sat}

	If loaded, \textsf{cleveref} can use its optional
	\verb|\label[problem]{sat}| syntax to
...
\namedpara{Rightflushed}[abv=RIGHT, flush=right]\\
\namedpara{Cref Satisfiability}[abv=SAT]\label[problem]{sat}\\
If loaded, \textsf{cleveref} can use its optional \verb|\label[problem]{sat}| syntax to reference \cref{sat}. This example assumes that the \verb|problem| category has been defined in the preamble with \verb|\crefname{problem}{problem}{problems}|.
\end{tcolorbox}
