...
\namedpara{Cref Satisfiability}
	...
	reference \cref{sat}. This example assumes that the 
	\verb|problem| category has been defined in the preamble 
	with \verb|\crefname{problem}{problem}{problems}|.
  \end{verbatim}
\begin{center}$\downarrow$\end{center}\Vskp
\namedpara{Indented}[abv=IND, indent=6em]\\
\namedpara{Surrounded}[abv=SUR, before=$>>>$ , after=$<<<$ ]\\
...
If loaded, \textsf{cleveref} uses the \verb|cref| key to reference \cref{sat}. This example assumes that the \verb|problem| category has been defined in the preamble with \verb|\crefname{problem}{problem}{problems}|.
\end{tcolorbox}
