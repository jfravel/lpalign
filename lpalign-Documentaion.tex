...
The newer \verb|namedpara| command leaves \verb|paratitle| untouched for compatibility but replaces the optional-argument convention with key-value settings. It takes a mandatory title and currently supports the keys \verb|abv|, \verb|indent| and \verb|cref|. The legacy-style key \verb|abbreviation| is also accepted as an alias for \verb|abv|. A single optional argument with no \verb|=| sign is interpreted as the abbreviation. The default indent is \verb|\HSkp|.
\begin{tcolorbox}
  \begin{verbatim}
...
Paragraph \ref{npara3} is a named paragraph.
\end{tcolorbox}
If \textsf{cleveref} is loaded, the \verb|cref| key is passed to \verb|\refstepcounter| as the reference type override for that paragraph.
