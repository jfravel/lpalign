...
Currently supported optional keys include 
	...
	\item \verb|premble| -- arbitrary code before the title
	\item \verb|flush| -- centers or right flushes the paragraph title via \verb|center| or \verb|right| -- respectively.
	\item \verb|cref| -- passes value to \verb|\refstepcounter| as the reference type override for that paragraph if \textsf{cleveref} is loaded.
	\end{itemize}
...
\namedpara{Premble Paragraph}
                     [abv=preamb, preamble=$\sim\sim\sim$]
...
reference the paragraph \cref{sat}
  \end{verbatim}
\begin{center}$\downarrow$\end{center}\Vskp
\namedpara{Indented Paragraph}[abv=indent, indent=6em] \\
\namedpara{Premble Paragraph}[abv=preamb, preamble=$\sim\sim\sim$] \\
\namedpara{Rightflushed Paragraph}[abv=right, flush=right] \\
\namedpara{Cref Satisfiability}[abv=SAT, cref=Problem]\label{sat} \\
If loaded, \textsf{cleverref} uses the \verb|cref| key to reference the paragraph \cref{sat}
\end{tcolorbox}

The compatability command \verb|paratitle| is left untouched from previous versions.
