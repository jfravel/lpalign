...
\date{Corresponds to \textsf{lpalign} v$1.1$, dated January $30^\textrm{th}$ 2026}
\author{Jamie Fravel}
\affil{jamiefravel@me.com}
...
\textsf{lpalign} is an environment macro built on the \textsf{alignat} environment from the \textsf{amsmath} package which helps to format mathematical programming problems aestetically so that they are legible in code as well as in the final pdf. Its primary function is to automatically embed the objective function in a zero-width box so that it's width does not interfere with the constraints spacing or centering.
...
The lengths \verb|\straise| and \verb|\objraise| are \verb|1/4\baselinskip| so that, if the corresponding vector has $n$ entries, \verb|n\straise| should be about halfway above vertical center of the matrix.
...
\begin{tcolorbox}
  ...
  Non-negetivity is enforced by \eqref{nneg}.
...
The \verb|namedsubeqs| environment will use a given name as the first index in any enclosed equation reference tag. A second index is appended as in a traditional \verb|subequations| environment.
    \begin{itemize}
    \item[M.] a name or abbreviation.
    \item[O.] an optional style for the second index (defaults to \verb|\alph|).
    \end{itemize}
...
Other common tag styles include \verb|\roman| and \verb|\arabic|.
\begin{tcolorbox}
  \begin{verbatim}
...
\VTrg
\begin{multicols}{2}
    \begin{namedsubeqs}{EQ}[\itshape\roman]
    \begin{align}
      A\mathbf{x} = \mathbf{b}   \\
      C\mathbf{y} = \mathbf{d}
    \end{align}
    \end{namedsubeqs}
    \begin{namedsubeqs}{EQ}[\scshape\alph]
    \begin{align}
      A\mathbf{x} = \mathbf{b}   \\
      C\mathbf{y} = \mathbf{d}
    \end{align}
...
Also available is \verb|\sublabel|, \verb|\subref| and \verb|\subeqref| which work like \verb|\label|, \verb|\ref| and \verb|\eqref| but only reference the secondary, sub-index of the tag. The \verb|\sublabel| macro includes \verb|\label|, so we don't need both.
\begin{tcolorbox}
  \begin{verbatim}
    \begin{namedsubeqs}{SQ}[\itshape\roman]
    \begin{align}
      A\mathbf{x} = \mathbf{b}   \sublabel{sq:1}\\
      C\mathbf{y} = \mathbf{d}   \sublabel{sq:2}
    \end{align}
    \end{namedsubeqs}

We can reference both (\subref{qs:1}-\subref{qs:2})
or just \subeqref{sq:1} or even \eqref{sq:2}.
  \end{verbatim}
\Vtrg
\begin{center}$\downarrow$\end{center}
\vtrg
    ...
    \end{align}
    \end{namedsubeqs}
We can reference both (\subref{sq:1}-\subref{sq:2}) 
or just \subeqref{sq:1} or even \eqref{sq:2}.
\end{tcolorbox}
Unfortunately, the \textsf{hyperref} links seem to be broken sometimes. I haven't been able to find this bug. The \verb|\sublabel| macro is designed around the \verb|namedsubeq| environment; it will not work properly in a traditional \verb|subequations| environment. Users should probably only use \verb|\sublabel| on equations they actually intend to \verb|\subref|.
...
\begin{center}$\downarrow$\end{center}
    \paratitle{Non-Linear Program}[NLP]\label{para:NLP}
    \begin{namedsubeqs}{NLP}[\itshape\roman]
    \begin{lpalign}{Minimize}{f(\mathbf{x})      \label{NLP.0}}
        g_i(\mathbf{x}) &\GE 0 &&\forall\ i\in I \label{NLP.1} \\&&
        h_j(\mathbf{x}) &\EQ 0 &&\forall\ j\in J \label{NLP.2} \\&&
            \mathbf{x}  &\GE 0                \sublabel{NLP.3}
...
Also included are font-relative version of the common spacing commands \verb|\quad| and \verb|\qquad| and a few more for good measure:
...
\section{Known Bugs}
\begin{itemize}
	\item[$\boxtimes$] Spacing macros \verb|\quad|and \verb|\qquad| overwrite default, absolute behavior with font-relative spacing. Fixed by renaming macros to \verb|\Hskp| and \verb|\HSkp|; v1.1.
	...
	\item[$\boxdot$] The \verb|\sublabel|, \verb|\subref| and \verb|\subeqref| macros do not work properly without the \textsf{hyperref} package 
	
	\item[$\boxdot$] The \verb|\sublabel|, \verb|\subref| and \verb|\subeqref| macros do not work outside of a \textsf{namedsubeqs} environment.
	
	\item[$\boxdot$] The \verb|\sublabel|, \verb|\subref| and \verb|\subeqref| macros sometimes hyperlink to the wrong tag.
\end{itemize}
...
\section{Version History}
	...
	\item[v1.1] Jan. 30, 2026. Bug Fix. Renamed quick spacing macros to maintain compatability with standard \TeX. Added negative, vertical and negative-vertical versions. Added summation spacing macros. Added clause alignment arguments to the \textsf{lpalign} environment.
	\end{itemize}
